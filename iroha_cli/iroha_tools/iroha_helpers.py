...
def add_peer(ip_address, peer_key):
    ...
    tx = iroha.transaction([iroha.command("AddPeer", peer=peer)])
    ic.sign_transaction(tx, user_private_key)
    send_transaction_print_status_and_return_result(tx)
...
def grant_account_tx_history_permission(account_id):
    tx = iroha.transaction(
        [
            iroha.command(
                "GrantPermission", account_id=account_id, permission=can_get_my_acc_txs
            )
        ]
    )
    ic.sign_transaction(tx, user_private_key)
...
def create_new_asset(asset, domain, precision):
    tx = iroha.transaction(
        [
            iroha.command(
                "CreateAsset", asset_name=asset, domain_id=domain, precision=precision
            )
        ]
    )
    ic.sign_transaction(tx, user_private_key)
...
def create_atomic_batch_transaction(
    user_1, user_2, asset_1, asset_2, asset_1_qty, asset_2_qty
):
    tx_1 = iroha.transaction(
        [
            iroha.command(
                "TransferAsset",
                src_account_id=user_1,
                dest_account_id=user_2,
                asset_id=asset_1,
                amount=asset_1_qty,
            )
        ],
    ...
    tx_2 = iroha.transaction(
        [
            iroha.command(
                "TransferAsset",
                src_account_id=user_2,
                dest_account_id=user_1,
                asset_id=asset_2,
                amount=asset_2_qty,
            )
        ],
...
def subtract_asset_qty(asset_id, qty):
    ...
    """
    tx = iroha.transaction(
        [iroha.command("SubtractAssetQuantity", asset_id=asset_id, amount=qty)]
    )
    ic.sign_transaction(tx, user_private_key)
    send_transaction_print_status_and_return_result(tx)
